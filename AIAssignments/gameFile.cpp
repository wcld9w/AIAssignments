  ...
  for (int i = 0; i < boardSize; i++)
  {
    for (int j = 0; j < boardSize; j++)
    {
      ...
      if (board.getCordChar(cords(i,j)) == 'C')
      {
        ...
        //This is how we determine which characters we want to grab for this direction
        string importantCharactersCardinalList = "#@";
        for (int k = 0; k < 4; k++)
        {
          char tempChar;
        ...
        string importantCharactersBothList = "C";
        for (int k = 0; k < 8; k++)
        {
          char tempChar;
          //Switch case to actually grab the correct cordinate
